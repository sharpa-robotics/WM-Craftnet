...
def _default_checkpoint_map_location():
    # Always remap serialized CUDA tensors to an existing local device.
    # This avoids errors like "deserialize object on CUDA device 1"
    # when current machine only has cuda:0.
...
def safe_load(filename, map_location=None):
    if map_location is None:
        map_location = _default_checkpoint_map_location()

    kwargs = {'map_location': map_location}
    # PyTorch 2.6+ defaults weights_only to True, which rejects checkpoints
    # containing objects such as NumPy scalars. Disable it when supported.
    try:
        import inspect
