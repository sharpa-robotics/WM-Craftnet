...
class A2CBase(BaseAlgorithm):
	def __init__(self, base_name, params):
		...
		# Keep a flat layout: <train_dir>/<prefix>/...
		self.experiment_dir = os.path.join(self.train_dir, self.prefix)
	...
	def get_action_values(self, obs):
		...
		world_model_enabled = self.config.get('world_model_enabled', False)
		if isinstance(processed_obs, dict):
			if world_model_enabled:
				# Keep dict observations so the actor can consume wm_feature.
				if 'student_obs' in processed_obs:
					processed_obs = copy.copy(processed_obs)
					processed_obs['obs'] = processed_obs['student_obs']
					processed_obs.pop('student_obs', None)
			else:
				if 'pointcloud' not in processed_obs:
					processed_obs = processed_obs.get('student_obs', processed_obs.get('obs'))
				elif 'student_obs' in processed_obs:
			...
			if self.has_central_value:
				...
				if self.distill:
					if 'pointcloud' in obs['obs']:
						processed_obs = self._preproc_obs({'obs': obs['obs'].get('student_obs', obs['obs'].get('obs')),
					 									   'pointcloud': obs['obs']['pointcloud']})
					else:
						processed_obs = self._preproc_obs(obs['obs'].get('student_obs', obs['obs'].get('obs')))
				else:
					processed_obs = self._preproc_obs(obs['obs'])
				input_dict = {
					'is_train': False,
		...
		while True:
			...
			if self.rank == 0:
				...
				if self.game_rewards.current_size > 0:
					...
					checkpoint_name = 'ep_' + str(epoch_num)
				...
				if epoch_num >= self.max_epochs:
					...
					self.save(os.path.join(self.nn_dir, 'ep_' + str(epoch_num)))
...
class ContinuousA2CBase(A2CBase):
	def __init__(self, base_name, params):
		...
		self.actions_low = torch.tensor(action_space.low, dtype=torch.float32).to(self.ppo_device)
		self.actions_high = torch.tensor(action_space.high, dtype=torch.float32).to(self.ppo_device)
	...
	def train_epoch(self):
		...
		c_losses = []
		b_losses = []
		pose_pred_pos_losses = []
		pose_pred_rot_losses = []
		pose_pred_weighted_losses = []
		pose_pred_rot_deg_losses = []
		tac_pred_losses = []
		tac_pred_accs = []
		entropies = []
		kls = []
		...
		for mini_ep in range(0, self.mini_epochs_num):
			...
			for i in range(len(self.dataset)):
				...
				if len(train_result) >= 12:
					pose_pred_pos_losses.append(train_result[9])
					pose_pred_rot_losses.append(train_result[10])
					pose_pred_weighted_losses.append(train_result[11])
				if len(train_result) >= 13:
					pose_pred_rot_deg_losses.append(train_result[12])
				if len(train_result) >= 15:
					tac_pred_losses.append(train_result[13])
					tac_pred_accs.append(train_result[14])
	...
	def train(self):
		...
		while True:
			epoch_num = self.update_epoch()
			step_time, play_time, update_time, sum_time, a_losses, c_losses, b_losses, pose_pred_pos_losses, pose_pred_rot_losses, pose_pred_weighted_losses, pose_pred_rot_deg_losses, tac_pred_losses, tac_pred_accs, entropies, kls, last_lr, lr_mul = self.train_epoch()
			total_time += sum_time
			frame = self.frame // self.num_agents
			...
			if self.rank == 0:
				...
				if len(pose_pred_pos_losses) > 0:
					self.writer.add_scalar('losses/pose_pred_pos_loss', torch_ext.mean_list(pose_pred_pos_losses).item(), frame)
				if len(pose_pred_rot_losses) > 0:
					self.writer.add_scalar('losses/pose_pred_rot_loss', torch_ext.mean_list(pose_pred_rot_losses).item(), frame)
				if len(pose_pred_rot_deg_losses) > 0:
					self.writer.add_scalar('losses/pose_pred_rot_deg', torch_ext.mean_list(pose_pred_rot_deg_losses).item(), frame)
				if len(pose_pred_weighted_losses) > 0:
					self.writer.add_scalar('losses/pose_pred_weighted', torch_ext.mean_list(pose_pred_weighted_losses).item(), frame)
				if len(tac_pred_losses) > 0:
					self.writer.add_scalar('losses/tac_pred_loss', torch_ext.mean_list(tac_pred_losses).item(), frame)
				if len(tac_pred_accs) > 0:
					self.writer.add_scalar('metrics/tac_pred_acc', torch_ext.mean_list(tac_pred_accs).item(), frame)
